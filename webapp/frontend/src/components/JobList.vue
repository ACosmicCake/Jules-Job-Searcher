    ...
    <ul v-if="jobsList.length > 0" class="jobs">
      <li v-for="job in jobsList" :key="job.id || job.job_url" class="job-item">
        ...
        <div class="job-description" v-if="job.description_text">
          ...
          <p class="description-text">{{ truncateDescription(job.description_text, 250) }}</p>
        </div>
...
const fetchJobs = async () => {
  ...
  try {
    ...
    jobsList.value = response.data || []; // Ensure it's an array
    if (response.data && response.data.length === 0 && skip.value > 0) {
